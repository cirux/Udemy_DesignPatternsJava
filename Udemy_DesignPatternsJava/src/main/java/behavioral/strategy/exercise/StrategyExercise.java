...
/**
 * A Strategy for determinant calulation in quadratic equation solution
 * @author ciro
 *
 */
...
/**
 * If the discriminant is negative, we return it as-is. This is Ok, since our
 * main API returns Complex numbers anyway
 *
 */
...
/**
 * If the discriminant is negative, the return value is NaN. NaN propagates
...
class QuadraticEquationSolver {
	...
	/**
	 * Calculates the canonical solution for a quadratic equation.
	 * 
	 * @param a
	 * @param b
	 * @param c
	...
	public Pair<Complex, Complex> solve(double a, double b, double c) {
		...
		Complex discriminantRoot = Complex.sqrt(discriminant);

		return new Pair<>(
				new Complex(-b, 0).plus(discriminantRoot).divides(new Complex(2, 0)).divides(new Complex(a, 0)),
				new Complex(-b, 0).minus(discriminantRoot).divides(new Complex(2, 0)).divides(new Complex(a, 0)));
	}
}
